(() => {
  ...
  function createButtonElement(title, text, textarea, id) {
    ...
    titleSpan.onclick = () => {
      if (titleSpan.contentEditable === "false") {
        ...
        textarea.style.height = `${
          Number(textarea.style.height.replace("px", "")) + 24
        }px`;
        textarea.focus();
      }
    };
    titleSpan.onblur = () => {
      titleSpan.contentEditable = "false";
      editStorage(titleSpan.innerHTML, titleSpan.dataset.id);
    };
    return button;
  }
  ...
  function editStorage(newTitle, id) {
    chrome.storage.sync.get(["presets"], function ({ presets }) {
